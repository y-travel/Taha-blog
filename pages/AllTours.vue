  ...
  <div :class="{ ltrdir: $i18n.locale === 'en' }">
    ...
    >
      ...
      <!-- <b-card class="col-sm-8 title list" img-right align="right">
                ...
                <div class="col-sm-1">
                    <img :src="`${tour.image[0].url}`" alt="img"
      ...
      <div class="col-sm-3" v-for="tour in tours" :key="tour.id">
        <b-card
          :title="`${tour.title}`"
          :img-src="`${tour.image[0].url}`"
          :img-alt="`${tours.slug}`"
          img-top
          tag="article"
          style="max-width: 20rem; text-align: right"
          class="mb-2"
        >
          <b-card-text>
            {{ tour.content }}
...
export default defineComponent({
  ...
  setup() {
    ...
    //
    onUpdated(async () => (loading.value = false));
    //
    return {
      tours,
      loading,
